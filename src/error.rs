...
use crate::{Colors, Dimension, MyError};
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            WSError::AtLeastValue(arg, value, num) => write!(
                f,
                "{e}: '{a}' value '{v}' must be at least '{n}'. The condition ({v} >= {n}) is false.\n\n\
                For more information, try '{h}'.",
                e = "Error".red().bold(),
                v = value.yellow(),
                a = arg.yellow(),
            ...
            WSError::AtMostValue(arg, value, num) => write!(
                f,
                "{e}: '{a}' value '{v}' must be at most '{n}'. The condition ({v} <= {n}) is false.\n\n\
                For more information, try '{h}'.",
                e = "Error".red().bold(),
                v = value.yellow(),
                a = arg.yellow(),
            ...
            WSError::InvalidSize(min_size, size, max_size) => write!(
                f,
                "{e}: invalid file size '{s}' bytes. The condition ({min} <= {s} <= {max}) is false.",
                e = "Error".red().bold(),
                min = min_size.green(),
                max = max_size.green(),
                s = size.yellow(),
            ),
            WSError::DisregardPath(path) => write!(
                f,
                "Disregard the path: '{p}'.",
