use crate::{Dimension, DimensionError, FileInfo, WallSwitchResult};
use blake3::Hasher;
...
/// Computes the BLAKE3 hash of multiple files concurrently using Rayon's global thread pool.
pub fn compute_hashes_parallel(files: &mut [FileInfo]) {
    files.par_iter_mut().for_each(|file_info| {
