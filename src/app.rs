...
pub fn run() -> WallSwitchResult<()> {
    ...
    if config.once {
        ...
        loop {
            match try_run_cycle(&config, &mut state)? {
...
fn try_run_cycle(config: &Config, state: &mut State) -> WallSwitchResult<CycleOutcome> {
    ...
    if valid_pool.len() >= needed {
        ...
        match set_wallpaper(&cycle_images, config) {
            Ok(_) => {
...
fn probe_and_validate_parallel(
    ...
    thread::scope(|scope| {
        for chunk in needs_update.chunks_mut(chunk_size) {
            scope.spawn(|| {
                chunk.iter_mut().for_each(|file_info| {
                    ...
                    if file_info.dimension.is_none() && file_info.update_dimension().is_ok() {
                        ...
                        // Lock stdout to prevent scrambled/interleaved terminal output across threads
