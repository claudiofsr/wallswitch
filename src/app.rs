...
/// Global control to ensure Rayon is initialized at most once.
static RAYON_INIT: Once = Once::new();

#[cfg(target_env = "gnu")]
unsafe extern "C" {
    /// Releases unused memory from the system allocator back to the operating system kernel.
    ///
    /// This is a non-standard GNU glibc extension that forces the allocator (`ptmalloc`)
    /// to release free memory pools (arenas) back to the OS. Calling this at the end of
    /// a wallpaper cycle helps prevent high virtual memory/RSS overhead during long
    /// periods of oclusal sleep.
    ///
    /// # Parameters
    ///
    /// * `pad` - The amount of free space to keep at the top of the heap. Typically `0`
    ///   is passed to maximize memory reclamation.
    ///
    /// # Returns
    ///
    /// Returns `1` if memory was successfully released back to the system, or `0` otherwise.
...
/// Core application entry point: coordinates arguments, system state, and execution cycles.
///
/// This function loads the environment, parses the command-line flags, merges settings with
/// the JSON configuration, handles terminal listing commands, and executes either a single switch
/// or starts the long-running application loop.
///
/// # Errors
///
/// Returns a [`WallSwitchResult`] if any critical configuration, state loading, or OS backend error occurs.
        ...
        loop {
            match try_run_cycle(&config, &mut state, &env)? {
                CycleOutcome::Success => {
                    // Force glibc allocator to return free memory back to the kernel
                    #[cfg(target_env = "gnu")]
                    unsafe {
                        malloc_trim(0);
                    }
...
/// Configures the global Rayon thread pool size based on the configured CPU percentage.
///
/// This uses a static [`Once`] lock to guarantee that `build_global`
/// is called at most once by our application. If Rayon has already been
/// initialized (e.g., implicitly by a test suite or another parallel task),
/// the error is treated as a non-fatal warning to prevent unnecessary crashes.
fn init_rayon_thread_pool(percent: u8, verbose: bool) {
    ...
    RAYON_INIT.call_once(|| {
        ...
        let num_threads = ((total_cores * percent as usize) / 100).max(1);
    ...
    if let Some(err_msg) = init_err
        && verbose
    {
        // Log as a non-fatal warning since the application can still fall back
        // to the already-initialized active thread pool.
        eprintln!(
            "{} Rayon thread pool was already started.
            Active configuration could not be changed: {}",
            "Note:".yellow().bold(),
            err_msg
        );
    }
...
/// Scans configured directories to collect files with Smart Caching and Visual Deduplication.
///
/// This function matches discovered file metadata (file size and mtime) against the persistent state
/// cache. If a match is found, the cached BLAKE3 hash and spatial dimensions are reused instantly.
/// Otherwise, a deep BLAKE3 hash is calculated concurrently, and visual duplicate files are discarded.
///
/// # Errors
///
/// Returns a [`WallSwitchResult`] if reading directories or performing filesystem operations fails.
...
/// Prints application startup details, package metadata, and recognized background tools.
///
/// # Errors
///
/// Returns a [`WallSwitchResult`] if terminal or config printing fails.
...
/// Coordinates a single wallpaper cycle execution using parallelized quorum check procedures.
///
/// This method collects unseen candidate images, validates them concurrently against dimension/size limits
/// across logical CPU threads, compiles the monitor-specific canvases, and updates the desktop background.
///
/// # Errors
///
/// Returns a [`WallSwitchResult`] if a system or desktop-environment communication error occurs.
fn try_run_cycle(
    config: &Config,
    state: &mut State,
    env: &Environment,
...
/// Iteratively accumulates candidate files into batches, validating them concurrently.
///
/// It processes validation requests across the calculated thread batch sizing to avoid I/O bottlenecks.
...
/// Reset history state and retries the cycle when the candidate pool is exhausted.
...
/// Filters all discovered image collections against active history, shuffling the layout result.
///
/// # Errors
///
/// Returns [`WallSwitchError::NoImages`] if no files are discovered on disk.
...
/// Displays formatted paths and metadata parameters of collected files in standard output.
...
/// Checks and updates spatial metadata for files, querying the state cache or performing image scans.
