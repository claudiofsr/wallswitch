...
pub use self::{
    config::*, desktops::*, dimension::*, environment::*, error::*, fileinfo::*, pids::*,
    traits::*, walkdir::*,
};
...
pub fn get_images(config: &Config) -> MyResult<Vec<FileInfo>> {
    ...
    if images.is_empty() {
        ...
        let error = WSError::NoImages(dirs);
        eprintln!("{error}");
    ...
    if nimages < config.monitor.into() {
        let dirs: Vec<PathBuf> = images.iter().map(|f| f.path.clone()).collect();
        let error = WSError::InsufficientImages(dirs, nimages);
        eprintln!("{error}");
        //return Err(error.into());
        std::process::exit(1);
    }
