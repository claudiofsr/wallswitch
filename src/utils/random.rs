    ...
    R: RandomCast,
{
    ...
    let raw_val = if min_val >= max_val {
        ...
        if range_diff == u64::MAX {
            rand::<u64>()
        } else {
            min_val + rand::<u64>() % (range_diff + 1)
        }
    };
...
/// Generates a random integer value in the specified range (min, max) inclusive safely.
///
/// # Errors
///
/// Returns a [`WallSwitchError::MinMax`] if the minimum boundary exceeds the maximum.
    ...
    R: RandomCast,
{
    if min > max {
        ...
        let raw_val = if range_diff == u64::MAX {
            rand::<u64>()
        } else {
            min + rand::<u64>() % (range_diff + 1)
        };
