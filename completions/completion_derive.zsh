...
'--min-size=[Set a minimum file size (in bytes) for searching image files]:MIN_SIZE:_default' \
'-B+[Set a maximum file size (in bytes) for searching image files]:MAX_SIZE:_default' \
'--max-size=[Set a maximum file size (in bytes) for searching image files]:MAX_SIZE:_default' \
'-d+[Set the minimum dimension that the height and width must satisfy]:MIN_DIMENSION:_default' \
'--min-dimension=[Set the minimum dimension that the height and width must satisfy]:MIN_DIMENSION:_default' \
'-D+[Set the maximum dimension that the height and width must satisfy]:MAX_DIMENSION:_default' \
'--max-dimension=[Set the maximum dimension that the height and width must satisfy]:MAX_DIMENSION:_default' \
...
'-n+[Set a custom minimum iteration limit for escape-time fractal calculations]:MIN_ITERATIONS:_default' \
'--effects-min-iterations=[Set a custom minimum iteration limit for escape-time fractal calculations]:MIN_ITERATIONS:_default' \
'-N+[Set a custom maximum iteration limit for escape-time fractal calculations]:MAX_ITERATIONS:_default' \
...
'--pictures-per-monitor=[Set number of pictures (or images) per monitor \[default\: 1\]]:PICTURES_PER_MONITOR:_default' \
...
'-t+[Limit the maximum execution threads used by parallel tasks]:PERCENT:_default' \
'--max-threads-percent=[Limit the maximum execution threads used by parallel tasks]:PERCENT:_default' \
'-c[Read the configuration file and exit the program]' \
'--config[Read the configuration file and exit the program]' \
'-1[Run a single wallpaper update cycle and exit]' \
'--once[Run a single wallpaper update cycle and exit]' \
'-s[Sort the images found]' \
'--sort[Sort the images found]' \
'-r[Run without applying the wallpapers (simulation mode)]' \
